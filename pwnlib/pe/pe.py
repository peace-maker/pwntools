...
class PE(PEFile, Binary):
    ...
    def debug(self, argv=[], *a, **kw):
        ...
        Debug the ELF with :func:`.windbg.debug`.

        Arguments:
            argv(list): List of arguments to the binary
            *args: Extra arguments to :func:`.windbg.debug`
            **kwargs: Extra arguments to :func:`.windbg.debug`

        Returns:
            :class:`.tube`: See :func:`.windbg.debug`
        """
        import pwnlib.windbg
        return pwnlib.windbg.debug([self.path] + argv, *a, **kw)
